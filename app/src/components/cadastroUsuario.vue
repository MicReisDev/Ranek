...
export default {
  ...
  methods:{
    enviarDados(){
        ...
        this.logarUsuarioCriado()
      },
      async logarUsuarioCriado(){
     
        if(this.$store.state.login === false){
             try {
            await this.$store.dispatch("criarUsuario", this.$store.state.usuario);
            await this.$router.push({name:"usuario"})
            await this.$store.dispatch("getUsuario",{usuario:this.$store.state.usuario.email,senha:this.$store.state.usuario.senha});
            if(this.$store.state.login === true){
            await this.$store.commit("MUDAR_MENSAGEM","Usuário criado com sucesso!" )
            }} catch(error){
              console.log(error)
            }
        }
